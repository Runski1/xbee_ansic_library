...
int xbee_scan_dump_response(xbee_dev_t *xbee, const void FAR *raw,
    ...
    if (resp->header.command.w == as.w) {
        if (XBEE_AT_RESP_STATUS(resp->header.status) == XBEE_AT_RESP_SUCCESS) {
            ...
            if (scan_length < 1) {
                puts("Scan complete");
            } else if ((xbee->hardware_series & XBEE_HW_SERIES_MASK) ==
                ...
                for (i = 0; i < scan_length; i++) {
                    ...
                    if (!isprint(c) && !isspace(c)) {
                        printf("Unexpected nonprintable byte in Cellular AS "
                               "response: index %u, ASCII 0x%02X\n",
                               i, c);
                        return 0;
                    }
                    putchar(c == '\r' ? '\n' : c);
                ...
                case XBEE_SCAN_TYPE_WIFI: {
                    ...
                    if (ssid_length < 1) {
                        ...
                        printf(
                            "CH%2u  Enc: %-4s  LM:%3udBm  ID:[%.*s]\n",
                            atas->wifi.channel,
                            xbee_wifi_encryption_name(atas->wifi.security_type),
                            atas->wifi.link_margin, ssid_length,
                            atas->wifi.ssid);
                    }
                    break;
                }
